    ...
    while True:
        ...
        match user_args[0]:
            case "create_table":
                if len(user_args) > 1:
                    if len(user_args) > 2:
                        new_meta = core.create_table(
                            metadata, user_args[1], user_args[2:]
                        )
                ...
                if len(user_args) > 4:
                    if user_args[1] == "into" and user_args[3] == "values":
                        ...
                        if len(table_values) == len(metadata[table_name].keys()) - 1:
                            ...
                            print("Число переменных не соответствует "
            ...
            case "select":
                if len(user_args) == 7:
                    ...
                    if (user_args[1] == "from" and
                        user_args[3] == "where" and
                        user_args[5] == '='
                        ):
                        table_name = user_args[2]
                        ...
                        if user_args[4] not in col_names:
                            ...
                            '''
                            Код ниже берет тип данных preferred_type
                            по данным нужного столбца и пытается
                            перевести строку "name = value",
                            задаваемую пользователем, в {"name": value}.
                            Если удалось, выполнение операции продолжается.
                            '''
                            col_index = col_names.index(user_args[4])
                            preferred_type = list(
                                metadata[table_name].keys()
                                )[col_index].split(sep=':')[1]
                            clause, success = parser.parse(
                                user_args[4] + '=' + user_args[6], preferred_type)
                            if success:
                                # select_data = core.select(table_data, clause)
                                select_data = select_cached(
                                    str(user_args), table_data, clause)
                                core.print_table(metadata, table_name, select_data)
                            else:
                                print(f"Значение {user_args[6]} нельзя использовать "
            ...
            case "update":
                if len(user_args) == 10:
                    if (user_args[2] == "set" and
                        user_args[4] == '=' and
                        user_args[6] == "where" and
                        user_args[8] == '='
                        ):
                        ...
                        if user_args[3] not in col_names:
                            ...
                            '''
                            Код ниже берет тип данных preferred_set_type
                            и preferred_where_type для обоих условий
                            по данным нужных столбцов и пытается
                            перевести строку "name = value",
                            задаваемую пользователем, в {"name": value}.
                            Если оба введенных пользователем условия
                            корректны, выполнение операции продолжается.
                            '''
                            set_index = col_names.index(user_args[3])
                            preferred_set_type = list(
                                metadata[table_name].keys()
                                )[set_index].split(sep=':')[1]
                            set_clause, set_success = parser.parse(
                                user_args[3] + '=' + user_args[5], preferred_set_type)
                            where_index = col_names.index(user_args[7])
                            preferred_where_type = list(
                                metadata[table_name].keys()
                                )[where_index].split(sep=':')[1]
                            where_clause, where_success = parser.parse(
                                user_args[7] + '=' + user_args[9], preferred_where_type)
                            if not set_success:
                                print(f"Значение {user_args[3]} нельзя использовать "
                                      f"для столбца типа {preferred_set_type}.")
                            elif not where_success:
                                print(f"Значение {user_args[7]} нельзя использовать "
